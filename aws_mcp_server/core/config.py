...
class ServerConfig:
    ...
    transport: str = field(
        default_factory=lambda: os.getenv("AWS_MCP_TRANSPORT", "stdio")
    )
    debug: bool = field(
        default_factory=lambda: os.getenv("AWS_MCP_DEBUG", "false").lower() == "true"
    )
    log_file: str = field(
        default_factory=lambda: os.getenv("AWS_MCP_LOG_FILE", "logs/aws_mcp_server.log")
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.transport not in ("stdio", "sse"):
            raise ValueError(
                f"Transport must be 'stdio' or 'sse', got {self.transport}"
            )
...
class AWSConfig:
    ...
    max_concurrent: int = field(
        default_factory=lambda: int(os.getenv("AWS_MCP_MAX_CONCURRENT", "10"))
    )
    max_results: int = field(
        default_factory=lambda: int(os.getenv("AWS_MCP_MAX_RESULTS", "1000"))
    )
    timeout_seconds: int = field(
        default_factory=lambda: int(os.getenv("AWS_MCP_TIMEOUT", "30"))
    )
    enable_pagination: bool = field(
        default_factory=lambda: os.getenv("AWS_MCP_ENABLE_PAGINATION", "true").lower()
        == "true"
    )
...
class VectorStoreConfig:
    ...
    enabled: bool = field(
        default_factory=lambda: os.getenv("ENABLE_VECTOR_STORE", "true").lower()
        == "true"
    )
    db_path: str = field(
    ...
    collection_name: str = field(
        default_factory=lambda: os.getenv("COLLECTION_NAME", "aws_docs")
    )
